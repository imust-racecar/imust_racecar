...
public:
    ...
    void path_complete_callback(const ros::TimerEvent &) {
        ...
        if (path_msg.poses.size() * 0.05 < goal_radius)
            can_start = false;
    }
...
private:
    geometry_msgs::PoseStamped transform_from_map_to_car(const geometry_msgs::PoseStamped data) {
        ...
        try {
            tf_listener.transformPose("base_link", ros::Time(0) , data, "map", pose);
    ...
    double get_gas_input() {
        ...
        double delta = 0;
        double last_y;
        for (int i = 10, j = 0; i < path_msg.poses.size() && j < 40; i++, j++) {
            auto pose = transform_from_map_to_car(path_msg.poses[i]);
            auto y = pose.pose.position.y;
            if (j == 0) last_y = y;
            if (j < 20) delta += fabs(y - last_y);
            delta += fabs(y - last_y);
            last_y = y;
        }
