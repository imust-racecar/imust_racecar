...
L1Controller::L1Controller() {
    ...
    pn.param("goal_radius", goalRadius, 1.2);
    ...
    can_run_circle_sub = n_.subscribe("/can_run_circle", 1, &L1Controller::canRunCircleCB, this);
    ...
    destination_timer = n_.createTimer(ros::Duration(1.0/controller_freq), &L1Controller::destinationLoopCB, this);
...
void L1Controller::destinationLoopCB(const ros::TimerEvent&) {
    static geometry_msgs::PoseStamped goal_msg[4];
    ...
    if (is_first) {
        ...
        goal_msg[1].pose.position.x = 4.5, goal_msg[1].pose.position.y = -2.3;
        goal_msg[2].pose.position.x = 1.0, goal_msg[2].pose.position.y = -2.3;
        goal_msg[3].pose.position.x = 0.0, goal_msg[3].pose.position.y = 0.0;
    }
    
    if (can_run_circle) {
        moveBase_pub.publish(goal_msg[goal_idx]);
    }
}

void L1Controller::canRunCircleCB(const geometry_msgs::Twist::ConstPtr& twistMsg) {
    if (twistMsg->linear.x != 0)
        can_run_circle = false;
    else
        can_run_circle = true;
}