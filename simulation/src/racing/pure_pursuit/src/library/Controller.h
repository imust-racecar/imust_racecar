...
//    ros system object
    ...
    ros::Subscriber path_sub, start_sub, cur_goal_sub, is_last_goal_sub;
...
//        initialize ros system object
        control_loop_timer = n_.createTimer(ros::Duration(1.0 / 50),
                                            &Controller::control_loop_callback, this);
        path_complete_timer = n_.createTimer(ros::Duration(1.0 / 20),
        ...
        is_last_goal_sub = n_.subscribe("/is_last_goal", 1,
                                    &Controller::is_last_goal_callback, this);
        cur_goal_sub = n_.subscribe("/cur_goal", 1,
                                    &Controller::cur_goal_callback, this);
...
//        ROS_WARN("gas: %.2f, steering: %.2f\n", cmd.linear.x, cmd.angular.z);
    ...
    void path_complete_callback(const ros::TimerEvent &) {
        if (!is_last_goal) return;

        geometry_msgs::PoseStamped pose;
        try {
            pose = transform_from_map_to_car(cur_goal);
        ...
        ROS_INFO("%.2f %.2f", x, y);
        if (x * x + y * y < goal_radius * goal_radius)
            can_stop = true;
    }
    ...
    void cur_goal_callback(const geometry_msgs::PoseStamped::ConstPtr & ptr) {
        cur_goal = *ptr;
    }

    void is_last_goal_callback(const std_msgs::Bool::ConstPtr & ptr) {
        is_last_goal = ptr->data;
    }
